#!/bin/bash
# IPA -> Slurm user sync
# Sources /config/ipa/ipa.conf at runtime on every execution so changes
# to ipa.conf take effect without restarting the container.
source /config/ipa/ipa.conf 2>/dev/null || true
